if object_id('tr_AnimationProduct_MultipleValueChanged') > 0
	drop trigger tr_AnimationProduct_MultipleValueChanged
go

create trigger tr_AnimationProduct_MultipleValueChanged
...
BEGIN
	...
	-- [newid() is there to avoid null compare issues]
	if @multipleNewID <> isnull(@multipleOldID, newid()) AND @multipleNewID IS NOT NULL
	begin
		...
		-- if new value is 1, we dont need to check anything
		if @multipleNewValue = 1
			return
