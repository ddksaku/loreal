IF OBJECT_ID (N'dbo.calculate_TotalAllocationUK', N'FN') IS NOT NULL
    DROP FUNCTION dbo.calculate_TotalAllocationUK;
GO

CREATE FUNCTION dbo.calculate_TotalAllocationUK (@AnimationProductID uniqueidentifier, @IDDivision uniqueidentifier)
RETURNS INT
WITH SCHEMABINDING
AS
BEGIN
	declare @TotalAllocationUK int


	SELECT @TotalAllocationUK = ISNULL(SUM(apd.AllocationQuantity),0)
	...
	return isnull(@TotalAllocationUK, 0)
END